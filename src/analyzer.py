...
class FrameAnalyzer:
    ...
    def solve(self):
        ...
        print(f">>> TEŞHİS: Toplam {applied_load_count} adet nodal yük sisteme işlendi.")
        print(f">>> TEŞHİS: Max Yük Değeri: {np.max(np.abs(F)) if len(F)>0 else 0}")
        ...
        if len(displacements) > 0:
            print(f">>> TEŞHİS: Max Deplasman: {np.max(np.abs(displacements))} m")
